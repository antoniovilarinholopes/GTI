...
declare function local:naive-bayes
  ( $model, $speech as xs:string ) {
   ...
   let $words_normalized_in_speech := ( for $word in $all_words_in_speech return if(string($word) = '') then () else fn:lower-case($word) )

  
    (:naive-bayes:)
   
    (:party prob:)
    let $total_partys_intervention := fn:sum( for $party in $model/model/party return $party/@size )
    ...
    let $word_probs := (	for $word_in_speech in $words_normalized_in_speech
			let $number_of_occ_in_model := fn:count(for $word in $model//word where $word[@token = $word_in_speech] return $word)
                        	return 
